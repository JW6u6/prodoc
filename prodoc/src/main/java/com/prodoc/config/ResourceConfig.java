...
public class ResourceConfig implements WebMvcConfigurer {
	...
	public void addResourceHandlers(final ResourceHandlerRegistry registry) {
		...
		registry.addResourceHandler("/download/**").addResourceLocations("file:///C:/image/")
				.setCacheControl(CacheControl.maxAge(1, TimeUnit.MINUTES)); // 접근 파일 캐싱 시간

		registry.addResourceHandler("/files/**").addResourceLocations(profilePath);

		registry.addResourceHandler("/dbFiles/**").addResourceLocations(dbattrPath);

		registry.addResourceHandler("/block/files/**").addResourceLocations(blockPath);
