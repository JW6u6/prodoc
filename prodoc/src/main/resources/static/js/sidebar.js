        ...
        .then((data) => {
            data.forEach((item, idx, dataList) => {
                if (item.level == 1) {
                    ...
                    if (item.wp == "page") {
                        //page
                        let workSite = document.querySelector(
                            '.Work[data-id="' + item.workId + '"]'
                        );
                        let text = `<div class= "Page" data-id="${item.pageId
                            }"  data-name="${item.workName}" data-level="2" data-number="${item.numbering
                            }">
                                    <span class="pageListShow">▼</span>
                                    <span class="pageName" draggable="true">${item.workName
                            }</span>
                                    ${item.caseId.substr(0, 2) != "DB"
                                ? '<span onclick="newPageModal(event)" class="add"><img class="plus" src="/images/plus.svg" width="15px" height="15px"></span>'
                                : ""
                            }
                        ...
                        if (item.caseId.substr(0, 2) == "DB")
                            temp_dbpageList.push(item.pageId);
                        workSite.children[4].insertAdjacentHTML("beforeend", text);
                    } else if (item.wp == "work") {
                        //work
                        let workSite = document.querySelector(
                            '.Work[data-id="' + item.parentId + '"]'
                        );
                    ...
                    //페이지
                    let pageSite = document.querySelector(
                        '.Page[data-id="' + item.parentId + '"]'
                    );
                    if (pageSite != null) {
                        //부모가 페이지
                        let text = `<div class= "Page" data-id="${item.pageId
                            }"  data-name="${item.workName}" data-level="3" data-number="${item.numbering
                            }" >
                                    <span class="pageListShow">▼</span>
                                    <span class="pageName" draggable="true">${item.workName
                            }</span>
                    ...
                    if (pageSite != null) {
                        ...
                        if (pageSite.parentElement.parentElement.className == "Work") {
                            text = `<div class= "Page" data-id="${item.pageId}"  data-name="${item.workName
                                }" data-level="4" data-number="${item.numbering}" >
                                    <span class="pageListShow">▼</span>
                                    <span class="pageName" draggable="true">${item.workName
                                }</span>
                                    ${item.caseId.substr(0, 2) != "DB"
...
function sideReloadEvent() {
    ...
    if (lastPageSession == null) {
        ...
        let isItLastPage = document.querySelector(
            '.Page[data-id="' + lastPageSession + '"]'
        );
    ...
    if (workClickSession != null) {
        ...
        document
            ...
            .forEach((workDiv) => workList.push(workDiv.dataset.id));
        ...
        for (let i = 0; i < workClickSession.length; i++) {
            ...
            if (autoToggle != null) {
                ...
                workClickSession = workClickSession.filter((data) =>
                    workList.includes(data)
                );
    ...
    if (pageClickSession != null) {
        ...
        document
            ...
            .forEach((workDiv) => pageList.push(workDiv.dataset.id));
        ...
        for (let i = 0; i < pageClickSession.length; i++) {
            ...
            if (autoToggle != null) {
                ...
                pageClickSession = pageClickSession.filter((data) =>
                    pageList.includes(data)
                );
            ...
            if (target.classList.contains("clicked")) {
                ...
                if (!check) {
                    ...
                    sessionStorage.setItem(
                        "workClickList",
                        JSON.stringify(workClickSession)
                    );
                }
                let pageDiv = target.parentElement.querySelector(".pageMain");
                pageDiv.classList.toggle("hide");
        ...
        Pages.addEventListener("click", function (e) {
            ...
            if (target.classList.contains("clicked")) {
                ...
                if (!check) {
                    ...
                    sessionStorage.setItem(
                        "pageClickList",
                        JSON.stringify(pageClickSession)
                    );
                }
                let pageDiv = target.parentElement.querySelector(".pageMain");
                pageDiv.classList.toggle("hide");
        ...
        fetch(URL, {
            ...
            .then((res) => {
                if (res.result) {
                    ...
                    //본문 내 페이지 타이틀 재설정
                    let pageTitleIs = document.querySelector(
                        ".app .pageHead input#TitleWid"
                    );
                    ...
                    //본문 내 데이터베이스 타이틀 재설정
                    document
                        .querySelectorAll(".app .database_case .db-block-header")
                        .forEach((dbTitleIs) => {
...
function dropPage(event) {
    ...
    if (offsetY < center + targetArea && offsetY > center - targetArea) {
        ...
        if (dragLevel < targetLevel) {
            exceptMove = targetItem.closest('.Page[data-level="' + targetLevel + '"]')
                .dataset.id;
        ...
        if (offsetY > center + targetArea) {
            ...
            let workId = targetItem.closest(".Work").dataset.id;
            let parentId =
                targetItem.parentElement.parentElement.parentElement.dataset.id;
            if (parentId == workId) {
                parentId = "";
            }
...
async function newWork() {
    ...
    document.querySelector('#invEmail').style.width = '325px';
    ...
    document.querySelector('#memberCheckArea').classList.add('hide');
...
document
    ...
    .children[2].addEventListener("click", function (e) {
...
function closeSideModal() {
    ...
    document
        .querySelector("#wsType")
        .removeEventListener("change", setTypeChange);
...
async function setWork(e) {
    ...
    if (subCheck == true) {
        autoCheckList();
        document.querySelector('#autoContain').style.width = '498px';
    }
    ...
    let listMem = document.querySelector('#memList');
    let btnAr = document.querySelector("#btnArea");
    let outMemAr = document.querySelector("#outmemArea");
    let memCkArea = document.querySelector('#memberCheckArea');
    document.querySelector('#invEmail').style.width = '505px';
    let members = await memberList(workId);
    let owner;
    ...
    document.querySelector("#ownArea > input").value = owner;
    ...
    document.querySelector('#invList').classList.add('hide');
    if (targetType.value == "TEAM") {
        ...
        //설정일때 보여야 하는거
        document
            .querySelector("#workEdit")
            .addEventListener("click", setTypeChange);

        document.querySelector("#teamEdit").addEventListener("click", async function (e) {
            document.querySelector("#teamEdit").classList.add('btnClicked');
            document.querySelector("#workEdit").classList.remove('btnClicked');

            await listWorkJoin(workId);
            memCkArea.classList.remove('hide');
            //멤버일때 보여야 하는거
            if (membAuth == "OWNER") {
                type.classList.add("hide");
        ...
        document.querySelector("#wsName").readOnly = false;
        delCheck.classList.add("hide");
        document.querySelector('#ownArea > input').readOnly = true;
...
async function setTypeChange(e) {
    document.querySelector("#teamEdit").classList.remove('btnClicked');
    document.querySelector("#workEdit").classList.add('btnClicked');
    ...
    if (membAuth == "OWNER") {
        ...
        delCheck.classList.add("hide");
        listMem.classList.add('hide');
        document.querySelector('#ownArea > input').readOnly = false;
        document.querySelector("#wsName").readOnly = false;
    } else if (membAuth == "MANAGER") {
        del.classList.add("hide");
        own.classList.add("hide");
...
function typeChange(e) {
    let invite = document.querySelector("#inviteUser"); //초대 부분
    document.querySelector('#invList').classList.remove('hide');
    let memberOption = document.querySelector("#memOption"); //멤버공개옵션(팀)
    let privateOption = document.querySelector("#privOption"); //비공개옵션(개인)
...
async function autoCheckList() {
    ...
    document
        .querySelector("#invEmail")
        .addEventListener("input", displayInputValue);
        ...
        if (!workId || workId.value == "") {
            ...
            if (subCheck == true) {
                ...
                if ( thisM == false && thisI == false && thisJ == true ) {
                    ...
                    let memCheckArea = document.querySelector("#memberCheckArea");
                    let beforeJoin = document.querySelector('#beforeJoin');
                    let trTag = document.createElement("tr");
                    let tdTag = document.createElement("td");
                    ...
                    memCheckArea.appendChild(beforeJoin);
                } else {
...
async function memberList(workId) {
    ...
    await fetch(url, {
        ...
        .then((result) => {
            ...
            document.querySelector('#allmember').textContent = result.length;
            for (let num of result) {
                ...
                if (num.auth != "OWNER") {
                    ...
                    authSelect.addEventListener("change", function (e) {
                        ...
                        let targetMail =
                            e.currentTarget.closest("tr").children[1].textContent;
                        let val = [];
                        val.push({
                            workId,
                            email: targetMail,
...
document
    .querySelector("#ownArea > input").addEventListener("change", changeOwner);

async function changeOwner(e) {
    let workId = document.querySelector("#wid").value;
    let checkManager = await memberList(workId);
    let mail = document.querySelector("#ownArea > input");
    let check = false;
    let owner = '';
    ...
    for (id of checkManager) {
        if (mail.value != '') {
            if (id.auth == 'MANAGER' && mail.value == id.email) {
                check = true;
                break;
            } else if (id.auth == 'NOMAL' && mail.value != id.email) {
                check = false;
            }
        } else {
