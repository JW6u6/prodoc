...
function pageList(wId, target) {
  ...
  fetch(url)
    ...
    .then(data => {
            data.forEach(item=> {
                let text = `<div class= "Page" data-id="${item.pageId}" data-name="${item.pageName}" data-level="2" ><span class="pageListShow">ㅇ</span><span class="pageName" draggable="true">  ${item.pageName}</span><span onclick="newPageModal(event)" class="add">
    ...
    .then((data) => {
      data.forEach((item) => {
        let text =
          ...
          item.pageName +
          "</span>" +
          '<span class="workIdVal">' +
          item.workId +
          "</span>" +
          ' <span onclick="newPageModal(event)" class="add"><img class="plus" src="/images/plus.svg" width="15px" height="15px"></span> <div class = "pageMain"></div> <div>';
        insertDiv.insertAdjacentHTML("beforeend", text);
      });
